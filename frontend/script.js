...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function updateSelectedPlayerCount() {
    selectedPlayerCountSpan.textContent = lineup.filter(slot => slot.player !== null).length;
...
`;
  ...
  function nextGameOfTournament() {
    ...
    matchResultsSection.classList.add("d-d-none"); // Hide detailed results
    ...
    // For AI opponents, generate a new AI opponent and transition to player selection
    const newAiTeam = generateAITeam().players;
    tournament.opponents.push(newAiTeam); // Add new AI opponent to the list

    // Reset user's team selection for the next game
    selectedPlayers = [];
    lineup = new Array(MAX_PLAYERS).fill(null);
    currentFormation = null;
    document
    ...
    playerSourceSelection.classList.remove("d-none"); // Show player source selection
    formationSelectionSection.classList.add("d-none"); // Hide formation selection
  ...
  viewDetailedResultsBtn.addEventListener("click", () => {
    gameResultsPage.style.display = "none";
    matchResultsSection.classList.remove("d-none");
    // Add a class to trigger the animation for stat bars
    analysisContent.parentElement.classList.add('analysis-visible');
  });
